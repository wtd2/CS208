...
\begin{document}
  ...
  \par It is easy to find that we use at most twice of binary search, with the initial range $O(n)$. Each time we use only query $O(1)$ times. Then the total times we need to query is $O(\log n)$.
